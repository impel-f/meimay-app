/**
 * ============================================================
 * MODULE 08: AI NAME ORIGIN GENERATOR (V15.0 - フォールバック強化版)
 * ============================================================
 */
...
async function generateOrigin() {
    ...
    const originDetails = combination.map(c => {
        const meaning = clean(c['意味']).split(/[。、]/)[0];
        return `【${c['漢字']}】：${meaning}`;
    }).join('\n');

    const prompt = `
名前「${givenName}」の由来を、以下の漢字データのみを使って、漢字の意味を生かして100〜150文字程度で簡潔に作成してください。

【禁止事項】
・「生命の誕生は～」「親の愛は～」などの前置きは不要
・名字についての言及も不要

【作成ルール】
・提示された漢字の意味に基づき、一人の人間としての成長や願いを直球で書く
...
function renderAIOriginResult(givenName, text, isFallback = false) {
    ...
    modal.innerHTML = `
        <div class="detail-sheet animate-fade-in flex flex-col items-center max-w-[420px]">
            ...
            <div class="w-full bg-[#fdfaf5] border border-[#eee5d8] rounded-[40px] p-8 mb-10 shadow-inner overflow-y-auto max-h-[50vh]">
