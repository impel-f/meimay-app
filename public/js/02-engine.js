...
function suggestReadingPatterns(reading) {
    console.log(`ENGINE: Suggesting patterns for "${reading}"`);
    const patterns = [];
    ...
    console.log(`ENGINE: Generated ${patterns.length} patterns`);
    return patterns;
}
...
function showReadingPatternSelector(reading) {
    console.log(`ENGINE: showReadingPatternSelector called with "${reading}"`);
    ...
    modal.onclick = (e) => {
        if (e.target === modal) closePatternSelector();
    };
            ...
            <div class="space-y-3 max-h-[60vh] overflow-y-auto">
                ${patterns.map((p, i) => `
                    <button onclick="selectPattern(${i})" class="w-full text-left p-4 rounded-2xl border-2 border-[#eee5d8] hover:border-[#bca37f] hover:shadow-md transition-all">
                        ...
                        <div class="text-xs text-[#a6967a]">${p.label}（${p.segments.length}文字）</div>
                    </button>
                `).join('')}
            </div>
    ...
    console.log("ENGINE: Pattern selector modal displayed");
}

function selectPattern(index) {
    console.log(`ENGINE: Pattern ${index} selected`);
    
    if (!window.currentReadingPatterns) {
        console.error("ENGINE: currentReadingPatterns not found");
        return;
    }
    ...
    console.log(`ENGINE: Segments set to`, segments);
...
function calculateKanjiScore(k) {
    ...
    if (typeof favorSimple !== 'undefined' && favorSimple && k['画数'] <= 10) score += 5;
    if (typeof favorComplex !== 'undefined' && favorComplex && k['画数'] >= 15) score += 5;
    
    return score;
}

console.log("ENGINE: Module loaded (v17.0 with pattern suggestion)");
