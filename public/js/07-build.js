...
function executeBuild() {
    ...
    console.log("BUILD: New result generated", currentBuildResult);
}
                ...
                <div class="inline-flex flex-wrap justify-center gap-2">
                    ${surnameStr ? `
                        <div class="reading-part-box">
                            ...
                            <span class="reading-text-main text-sm">${surnameStr}</span>
                        </div>
                    ` : ''}
                    <div class="reading-part-box">
                        <span class="reading-label-mini">名</span>
                        <span class="reading-text-main text-sm">${r.reading}</span>
                    </div>
                </div>
            </div>
...
function reselectSlot(slotIdx) {
    if (confirm(`${slotIdx + 1}文字目「${segments[slotIdx]}」を選び直しますか？\n現在の選択がリセットされます。`)) {
        ...
        console.log(`BUILD: Reselecting slot ${slotIdx}`);
    }
}
